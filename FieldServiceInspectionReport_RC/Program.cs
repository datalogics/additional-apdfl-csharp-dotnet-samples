...
internal static class Program
{
    public static int Main(string[] args)
    {
        try
        {
            ...
            using Document document = new() { Title = report.Title, Producer = "CreateFieldServiceInspectionReport using APDFL" };
            new PdfInspectionRenderer().Render(document, report, findings, paths.Images, paths.Output);
            Console.WriteLine($"Created {paths.Output}");
            return 0;
        }
        catch (Exception ex) when (ex is ArgumentException or InvalidDataException or IOException or JsonException or LibraryException)
        {
    ...
    private static void Validate(InspectionReport r, Paths paths)
    {
        ...
        foreach (Photo photo in r.Photographs)
        {
            ...
            if (string.IsNullOrWhiteSpace(photo.AltText)) throw new InvalidDataException($"Photograph needs altText: {photo.File}");
        }
    }
...
internal sealed record Photo(string File, string Caption, string AltText);
